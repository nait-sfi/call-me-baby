...
class Small_LLM_Model:
    ...
    def get_logits_from_input_ids(
        self, input_ids, past_key_values=None, new_token_ids=None
    ):
        if past_key_values is None:
            ...
            input_tensor = torch.tensor(
                [new_token_ids], device=self._device, dtype=torch.long
            )
        with torch.no_grad():
            out = self._model(
                input_ids=input_tensor, past_key_values=past_key_values, use_cache=True
            )
